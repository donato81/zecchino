...
- [ ] Leggere `docs/2 - coding plans/P09-coding-plan.md` per intero
- [ ] Prendere nota dell'ambiguità **AI1** (`handleAddFundsToGoal` va dichiarato **dopo** i 4 `useState` da cui dipende — non prima)
- [ ] Prendere nota dell'ambiguità **AI2** (`chartPeriod` non si rimuove nel Passo A: rimane in `App.tsx` fino al Passo C, poi si rimuove quando TypeScript lo segnala come inutilizzato)
- [ ] Prendere nota dell'ambiguità **AI3** (`soundSystem` e `hapticSystem` vanno importati direttamente in `ReportsTab.tsx` da `@/lib/sound-system` e `@/lib/haptic-system` se usati negli `onClick` inline)
- [ ] Prendere nota dell'ambiguità **AI4** (icone base: `Plus`, `Gear`, `Target`, `Info`, `PiggyBank` — aggiungere `PencilSimple`/`Trash` solo se compaiono nel JSX fuori dai componenti estratti)
- [ ] Prendere nota di **R1** (`setTimeout(300ms)` nella callback `BudgetAlertBanner` in `App.tsx` — non toccare mai)
- [ ] Prendere nota di **R4** (il Passo B dipende interamente dal Passo A completato con `tsc --noEmit` a zero errori)
- [ ] Prendere nota di **R7** (`SavingsGoalCard` usa `accounts={safeAccounts}` — non `visibleAccounts`)
...
- [ ] Individuare `setShowAccountDialog: (v: boolean) => void` (~riga 62) — ultima riga del blocco dialog account
- [ ] Aggiungere **dopo** quella riga e **prima** della chiusura `}` del tipo:
  ...
  ```
- [ ] Verificare che `Budget` e `SavingsGoal` siano già nell'import a riga 3 ✓

### A.2 Dichiarazione dei 4 `useState` in `AppDataProvider` (dopo riga ~86)

- [ ] Individuare `const [showAccountDialog, setShowAccountDialog] = useState(false)` (~riga 86)
...
- [ ] ⚠️ **AI1**: aggiungere **dopo** i 4 `useState` del passo A.2 (non prima — dipende da `setEditingSavingsGoal` e `setShowSavingsGoalDialog`):
  ```ts
...
- [ ] Individuare `setShowAccountDialog,` nell'oggetto `value` di `AppDataContext.Provider`
...
- [ ] Aprire `src/App.tsx`
- [ ] Individuare il blocco `const { ... } = useAppData()`
...
- [ ] Individuare e rimuovere ~riga 113: `const [showBudgetDialog, setShowBudgetDialog] = useState(false)`
- [ ] Individuare e rimuovere ~riga 114: `const [showSavingsGoalDialog, setShowSavingsGoalDialog] = useState(false)`
- [ ] ⚠️ **Non rimuovere** ~riga 115: `const [showKeyboardHelp, setShowKeyboardHelp] = useState(false)` — rimane locale in `App.tsx`
- [ ] Individuare e rimuovere ~riga 117: `const [editingBudget, setEditingBudget] = useState<Budget | undefined>()`
- [ ] Individuare e rimuovere ~riga 118: `const [editingSavingsGoal, setEditingSavingsGoal] = useState<SavingsGoal | undefined>()`
- [ ] ⚠️ **Non rimuovere** ~righe 120–122: `activeTab`, `chartPeriod`, `previousTab` — rimangono in `App.tsx` per ora (⚠️ **AI2**: `chartPeriod` verrà rimosso nel Passo C)

### A.7 Rimozione di `handleAddFundsToGoal` locale da `App.tsx` (~righe 130–133)

- [ ] Individuare e rimuovere la funzione locale:
  ```ts
  const handleAddFundsToGoal = (goal: SavingsGoal) => {
    setEditingSavingsGoal(goal)
    setShowSavingsGoalDialog(true)
  ...
  ```
- [ ] Verificare che il blocco `useEffect(() => { if (showDeleteDialog) { ... } }, [showDeleteDialog])` (~righe 124–128) **non venga rimosso**

### A.8 Verifica del Passo A

- [ ] Salvare entrambi i file
- [ ] Eseguire `npx tsc --noEmit` → zero errori TypeScript
- [ ] Verificare che `useAppData()` esponga `showBudgetDialog`, `editingBudget`, `showSavingsGoalDialog`, `editingSavingsGoal`, `handleAddFundsToGoal`
- [ ] Verificare che `App.tsx` non contenga più `useState` locali per `showBudgetDialog`, `showSavingsGoalDialog`, `editingBudget`, `editingSavingsGoal`
- [ ] Verificare che `App.tsx` non contenga più la funzione locale `handleAddFundsToGoal`
- [ ] ⚠️ **R1**: verificare che il blocco `BudgetAlertBanner` con `setTimeout(300ms)` in `App.tsx` sia invariato:
  ```tsx
  ...
  ```
- [ ] ⚠️ **R4**: non procedere al Passo B fino a zero errori TypeScript
...
- [ ] Creare il file `src/components/ReportsTab.tsx` vuoto
- [ ] Aggiungere import da `react`: `useState`, `useMemo`
- [ ] Aggiungere import da `@/context/AppDataContext`: `useAppData`
- [ ] Aggiungere import da `@/hooks/use-visible-data`: `useVisibleData`
- [ ] ⚠️ **R1**: aggiungere import da `@/lib/helpers`: `formatCurrency`, `calculateAccountBalance`, `getActiveBudgets` — importati **direttamente**, non via context
- [ ] ⚠️ **AI3**: aggiungere import da `@/lib/sound-system`: `soundSystem` (se usato negli `onClick` inline — verificare in B.8)
- [ ] ⚠️ **AI3**: aggiungere import da `@/lib/haptic-system`: `hapticSystem` (se usato negli `onClick` inline — verificare in B.8)
- [ ] Aggiungere import dei componenti estratti:
...
- [ ] Aggiungere import da `@/components/ui/card`: `Card`, `CardContent`, `CardHeader`, `CardTitle`
...
- [ ] ⚠️ **R6**: aggiungere import da `@phosphor-icons/react`: `Plus`, `Gear`, `Target`, `Info`, `PiggyBank`
- [ ] ⚠️ **AI4**: verificare durante B.8 se servono ulteriori icone (`PencilSimple`, `Trash`) — aggiungere solo se referenziate nel JSX fuori dai componenti estratti
- [ ] Verificare dal JSX se sono necessari ulteriori import da `@/components/ui/` (es. `Separator`, `Badge`) — aggiungere solo quelli referenziati

### B.2 Firma del componente

- [ ] Aprire `export function ReportsTab() {`
...
- [ ] Destructure da `useVisibleData()`:
  ...
  - `visibleTransactions`
  - `totalBalance`
- [ ] Verificare che **non** siano inclusi nella destructuring: `recentTransactions`, `groupedAccounts`, `filteredGroupedAccounts`, `editingAccount`, `showAccountDialog`, `editingTransaction`, `showTransactionDialog`

### B.4 Stato locale `chartPeriod`

- [ ] ⚠️ **Design §6** — dichiarare `chartPeriod` come `useState` locale (NON in AppDataContext):
  ```ts
...
- [ ] ⚠️ **Design §5.B** — dichiarare il `useMemo` che sostituisce le 4 chiamate `getActiveBudgets(safeBudgets)`:
  ```ts
...
- [ ] ⚠️ **Design §5.A** — dichiarare il `useMemo` che sostituisce la IIFE nella card "Totale Entrate":
  ```ts
...
- [ ] ⚠️ **Design §5.A** — dichiarare il `useMemo` che sostituisce la IIFE nella card "Totale Uscite":
  ```ts
...
- [ ] Aprire `src/App.tsx` e individuare il blocco `<TabsContent value="reports" ...>` (~riga 417)
- [ ] Individuare la chiusura `</TabsContent>` corrispondente (~riga 757)
- [ ] Copiare il blocco (~341 righe) come `return (...)` del componente

**Modifiche obbligatorie durante la copia:**

- [ ] ⚠️ **Design §5.B** — sostituire ogni `getActiveBudgets(safeBudgets)` con `activeBudgets` — **4 occorrenze**:
...
- [ ] ⚠️ **Design §5.A** — sostituire la IIFE nella card "Totale Entrate" (~righe 487–510 originali) con `topIncomeCategories.length > 0 ? ... : ...`
- [ ] ⚠️ **Design §5.A** — sostituire la IIFE nella card "Totale Uscite" (~righe 527–545 originali) con `topExpenseCategories.length > 0 ? ... : ...`
- [ ] ⚠️ **R7** — verificare esplicitamente che `SavingsGoalCard` riceva `accounts={safeAccounts}` — NON modificare con `visibleAccounts`
- [ ] ⚠️ **R1** — il blocco JSX estratto **non include** la callback `BudgetAlertBanner` (quella rimane in `App.tsx` con il `setTimeout(300ms)` intatto)
...
- [ ] Verificare: `grep "getActiveBudgets(safeBudgets)" src/components/ReportsTab.tsx` → zero risultati
- [ ] Verificare che le 2 IIFE siano state rimosse e sostituite con `topIncomeCategories`/`topExpenseCategories`
...
- [ ] Aprire `src/App.tsx`
- [ ] Aggiungere vicino all'import di `DashboardTab`:
  ```ts
...
- [ ] Individuare `<TabsContent value="reports" ...>` (~riga 417)
- [ ] Individuare la chiusura `</TabsContent>` corrispondente (~riga 757)
- [ ] Rimuovere l'intero blocco (~341 righe)
- [ ] Inserire al posto del blocco rimosso:
  ```tsx
  <ReportsTab />
  ```
- [ ] Verificare che `<DashboardTab />` e `<TransactionsTab />` rimangano al loro posto
- [ ] ⚠️ **R1** — verificare che il blocco `BudgetAlertBanner` con `onViewBudget` e `setTimeout(300ms)` rimanga invariato nella sua posizione in `App.tsx`

### C.3 Rimozione di `chartPeriod` da `App.tsx` (~riga 121)

- [ ] ⚠️ **AI2** — dopo la rimozione del blocco JSX, TypeScript segnala `chartPeriod` e `setChartPeriod` come non utilizzati
- [ ] Rimuovere la riga:
  ```ts
  const [chartPeriod, setChartPeriod] = useState<'week' | 'month' | '3months' | '6months' | 'year'>('month')
  ```
...
- [ ] Affidarsi agli errori TypeScript per identificare import diventati inutilizzati
- [ ] Rimuovere solo gli import che TypeScript segnala come inutilizzati
- [ ] ⚠️ Non rimuovere import speculativamente

...
export function useUserSettings(): UserSettingsState {
  ...
  useEffect(() => {
    ...
    // Wave A: Audio — read from prefs; if absent trigger one-shot KV migration
    ...
    // Wave B: Display — read from prefs; if absent trigger one-shot KV migration
    ...
    // Wave C: Screen reader — read from prefs; if absent trigger one-shot KV migration
